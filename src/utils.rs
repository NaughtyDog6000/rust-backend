...
use crate::{
    errors::CustomErrors,
    structs::{build_user, FriendRecord, FriendRequest, RelationshipStatusEnum, Token, User},
};
use axum::{extract::FromRequest, headers::Expires, http::StatusCode, Extension, Json};
use log::{error, info, warn};
use serde_json::json;
use sqlx::{database::HasValueRef, pool, Error, PgPool};
...
pub async fn get_user(
    ...
    if id.is_some() {
        ...
        let token_req = sqlx::query_as::<_, Token>(
            "SELECT * FROM tokens
                                WHERE token = $1; 
        ",
        )
        ...
        if token_req.is_err() {
            return Err(CustomErrors::TokenError);
        }
        ...
        res = sqlx::query_as::<_, User>("SELECT * FROM users WHERE username = $1")
...
pub async fn check_user_exists(username: &String, pool: &PgPool) -> bool {
    let query_res = sqlx::query("SELECT id, epoch_signup_time FROM users WHERE username = $1")
        .bind(&username)
        .fetch_optional(pool)
        .await;
...
pub async fn create_session_token(
    ...
    let insrt_rsp = sqlx::query(
        "INSERT INTO tokens (user_id, epoch_expiry_date, token)
                VALUES ($1,$2,$3);
    ",
    )
    .bind(user.id)
...
pub async fn check_token(pool: &PgPool, token: String) -> bool {
    ...
    let response = sqlx::query_as::<_, Token>(
        "SELECT * FROM tokens
...
pub fn check_password_regex(password: &String) -> bool {
...
pub fn check_username_regex(password: &String) -> bool {
    let reg: Regex = Regex::new(r"^[0-9A-Za-z_.]+$").unwrap();
    if reg.is_match(password) {
        return true;
    }
...
pub async fn get_friends(pool: &PgPool, user_id: i64) -> Result<Vec<FriendRecord>, String> {
...
pub async fn get_outgoing_friend_requests(user_id: i64) -> Result<Vec<FriendRequest>, String> {
    todo!();
    // return Err(String::from("NOT COMPLETE"));
}

pub async fn get_incoming_friend_requests(user_id: i64) -> Result<Vec<FriendRequest>, String> {
    todo!();
    // return Err(String::from("NOT COMPLETE"));
}

/// returns a tuple containing the vecs of the relationships (requested, requestee or friends)
pub async fn get_all_relationships(user_id: i64) -> Result<Vec<RelationshipStatusEnum>, String> {
    todo!();
    // return Err(String::from("NOT COMPLETE"));
}
...
pub async fn add_or_accept_friend(
    ...
    let status: Result<RelationshipStatusEnum, CustomErrors> =
        get_friend_status(pool, user_id, target_id).await;
